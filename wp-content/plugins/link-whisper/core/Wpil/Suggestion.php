...
class Wpil_Suggestion
{
    ...
    public static function ajax_get_post_suggestions(){
        ...
        if(empty($count)){
            update_user_meta(get_current_user_id(), 'wpil_same_category_selected', $same_category);
        }
    ...
    public static function ajax_update_suggestion_display(){
        ...
        $same_category = !empty(get_user_meta(get_current_user_id(), 'wpil_same_category_selected', true));
        ...
        if('outbound_suggestions' === $_POST['type']){
            ...
            if(empty($selected_categories) && !empty($categories)){
                $selected_categories = array_map(function($cat){ return $cat->term_id; }, $categories);
            }
            ...
            if(empty($selected_tags) && !empty($tags)){
                $selected_tags = array_map(function($tag){ return $tag->term_id; }, $tags);
            }

            $select_post_types = !empty($_POST['select_post_types'] && $_POST['select_post_types'] === 'true') ? 1 : 0;
            $post_types = Wpil_Settings::getPostTypes();
            $selected_post_types = (isset($_POST['selected_post_types']) && !empty($_POST['selected_post_types']) && is_array($_POST['selected_post_types'])) ? array_intersect($post_types, $_POST['selected_post_types']) : $post_types;
            $selected_post_types = (!empty($selected_post_types)) ? $selected_post_types: $post_types;
            $post_types = Wpil_Settings::getPostTypeLabels($post_types);
    ...
    public static function getTitleWords($post, $target = null, $keyword = null, $count = null, $process_key = 0)
    {
        ...
        if (!is_null($target)) {
            ...
            // if terms are to be scanned, but the user is restricting suggestions by term, don't search for terms to link to
            if (!empty(Wpil_Settings::getTermTypes()) && empty($_POST['same_category']) && empty($_POST['same_tag']) && empty($only_show_cornerstone)) {
    ...
    public static function trimTags(&$sentences, $with_links = false)
    {
        foreach ($sentences as $key => $sentence)
        {
            ...
            if (substr_count($sentence, '<a ') >  substr_count($sentence, '</a>') ||
                substr_count($sentence, '<ta ') >  substr_count($sentence, '</ta>')
            ) {
                unset($sentences[$key]);
                continue;
            }
    ...
    public static function getSuggestionPostTypes(){
        ...
        if( isset($_POST['select_post_types']) && !empty($_POST['select_post_types']) &&
            isset($_POST['selected_post_types']) && !empty($_POST['selected_post_types']))
        {
            // check to make sure the supplied post types are ones that are in the settings
            $potential_types = array_intersect($post_types, $_POST['selected_post_types']);
     ...
     * Pulls data from the $_POST variable
     * @return array
     **/
    public static function get_selected_categories(){
        ...
        return array_filter(array_map(function($cat){ return (int)$cat; }, $_POST['selected_category']));
    }
    ...
    public static function get_selected_tags(){
        ...
        return array_filter(array_map(function($tag){ return (int)$tag; }, $_POST['selected_tag']));
    }
}
