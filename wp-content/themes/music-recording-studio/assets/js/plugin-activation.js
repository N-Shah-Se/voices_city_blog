jQuery(document).ready(function($) {
    'use strict';
    var this_obj = music_recording_studio_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.music-recording-studio-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message music-recording-studio-plugin-install');
                button.addClass('button-primary music-recording-studio-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.music-recording-studio-recommended-plugins .music-recording-studio-activation-note').css('display','block');
    ...
    $(document).on('click', '.music-recording-studio-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .music-recording-studio-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .music-recording-studio-recommended-plugins').css('display','none');
    });
